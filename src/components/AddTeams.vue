<template>
  <div class="p-4">
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
      <img src="@/assets/cards.webp" alt="Cards" class="w-full h-24 object-cover" />
      <div class="p-4">
        ...
        <div v-for="i in teams" :key="i" class="mb-4">
          <label :for="`team-${i}`" class="block text-sm font-medium text-gray-700"
            >Time {{ i }}</label
          >
          <input
            type="text"
        ...
        <div class="mb-4">
          <label for="winningPoints" class="block text-sm font-medium text-gray-700"
        ...
        <button @click="submit" class="bg-blue-500 text-white px-4 py-2 rounded-sm">
          Iniciar Jogo
        </button>
      </div>
    </div>
...
const submit = (): void => {
  if (names.value.slice(0, teams.value).every((name) => !!name) && winningPoints.value > 0) {
    if (teams.value === 2) {
